...
> = {
  PAGE_VIEW: { icon: Eye, label: "Page View", color: "text-text-muted", bg: "bg-surface-elevated" },
  ...
  APPLICATION_SUBMIT: { icon: FileText, label: "Application", color: "text-accent", bg: "bg-accent-dim" },
  ...
  WEBINAR_CTA_CLICK: { icon: Link2, label: "Webinar CTA", color: "text-accent", bg: "bg-accent-dim" },
  URL_RULE_MATCH: { icon: Tag, label: "URL Rule", color: "text-yellow", bg: "bg-yellow-dim" },
  CUSTOM: { icon: Zap, label: "Custom", color: "text-text-muted", bg: "bg-surface-elevated" },
};
      ...
      <div className="space-y-1">
        ...
        {filtered.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3 transition-colors hover:border-border-bright"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[11px] text-text-dim">
                  ...
                  {source && (
                    <span className="rounded-full bg-surface-elevated px-1.5 py-0.5">
                      {source}
                    </span>
                  )}
